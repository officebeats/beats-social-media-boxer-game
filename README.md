...
The shipped art uses a custom default-palette fighter atlas for bodies and HUD portraits, with procedural arms retained so every rendered punch stays aligned to gameplay reach. Fight layers render at their native 40-pixel atlas width with no runtime downscaling. Explicit eye, ear, nose, cheek, beard, goatee, and braid clusters preserve face identity while three-tone shirt, chest, rib, abdominal, joint, glove-cuff, trunk-panel, thigh, sock, and boot shading keeps the larger fighters athletic and authored rather than minimal. A.B.'s compact fade, heavy brow, bright cheek plane, separated beard, tailored shirt, and purple equipment contrast with D. Great's tapered jaw, individual braid strands, sharper eye line, small goatee, bare torso, and green equipment. [`reference/art-direction-lock.png`](reference/art-direction-lock.png) is the canonical art target; regenerate the atlas after source edits with `python .\tools\build-fighter-atlas.py`.
